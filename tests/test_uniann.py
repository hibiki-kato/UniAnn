...
def make_spliced_fixture(directory, kind="donor"):
    length = 300
    sequence = ["A"] * length
    motifs = [(0, "ATG"), (60, "GT"), (150, "AG"), (260, "TAA")]
    gt = [(60, 100)]
    ag = [(150, 100)]
    if kind == "donor":
        motifs.append((51, "GT"))
    ...
    for position in range(length):
        ...
        if kind == "donor" and 52 <= position <= 60:
            values[4] = 1.0
        if 151 <= position <= 261:
            values[3] = 1.0  # E2 after a 92- or 101-base modeled intron
...
def make_three_exon_fixture(directory, mode):
    length = 400
    sequence = ["A"] * length
    motifs = [(0, "ATG"), (60, "GT"), (150, "AG"),
              (210, "GT"), (300, "AG"), (340, "TAA")]
    if mode == "duplicate":
        motifs.append((51, "GT"))
    else:
        motifs.append((350, "TAA"))
    for position, motif in motifs:
        sequence[position : position + len(motif)] = motif
    ...
    for position in range(length):
        ...
        if 2 <= position <= 60:
            values[1] = 1.0
        if (52 if mode == "duplicate" else 61) <= position <= 300:
            values[4] = 1.0 if mode in {"duplicate", "inclusion"} else 0.0
        if 151 <= position <= 210:
            values[3] = 1.0 if mode != "inclusion" else 0.0
        if 211 <= position <= 300:
            values[4] = 1.0 if mode != "inclusion" else 0.0  # second intron
        if 301 <= position <= 341:
            values[2] = 1.0 if mode != "inclusion" else 0.0
        if mode in {"skipping", "inclusion"} and 301 <= position <= 351:
            values[3] = 1.0
        emissions.append(values)

    if mode == "duplicate":
        gt = [(51, 95), (60, 100), (210, 100)]
        ag = [(150, 100), (300, 100)]
        stop = [(340, 100)]
    elif mode == "skipping":
        gt = [(60, 100), (210, 100)]
...
def test_legacy_golden():
    with tempfile.TemporaryDirectory() as temporary:
        ...
        emissions = [[0] * 5 for _ in sequence]
...
def test_alternative_donor_and_format():
    ...
    try:
        ...
        assert kept[0]["intron_chain"] == "51-150"
...
def test_alternative_acceptor():
    ...
    try:
        ...
        assert kept[0]["intron_chain"] == "60-159"
...
def test_cli_rejects_alt_options_without_mode():
    with tempfile.TemporaryDirectory() as temporary:
        sequence = "A" * 20
        inputs = write_inputs(temporary, "cli", sequence, [[0] * 5 for _ in sequence])
